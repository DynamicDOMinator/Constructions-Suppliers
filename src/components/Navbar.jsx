    ...
    <nav className="w-full bg-white py-4 px-6 md:px-12 flex flex-row-reverse items-center justify-between sticky top-0 z-50">
      ...
      <div className="flex  items-center gap-4" dir="ltr">
        ...
        {isAuthenticated ? (
            ...
            <div className="relative">
              ...
              {isProfileOpen && (
                <div className="absolute top-full left-0 mt-4 w-48 bg-white border border-gray-100 shadow-xl rounded-xl z-50 overflow-hidden font-tajawal text-right animate-in fade-in slide-in-from-top-2 duration-200" dir="rtl">
                  ...
                  {user?.type === 'engineer' ? (
                    <Link href={`/engineer/${user?.uuid || user?.id}`} className="block px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 transition-colors" onClick={() => setIsProfileOpen(false)}>زيارة الملف الشخصي</Link>
                  ) : (
                    <Link href="/dashboard" className="block px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 transition-colors" onClick={() => setIsProfileOpen(false)}>لوحة التحكم</Link>
                  )}
                  <Link href="/settings" className="block px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 transition-colors" onClick={() => setIsProfileOpen(false)}>الإعدادات</Link>
                  <button 
                    onClick={() => {
                      logout();
          ...
          <div className="hidden md:flex items-center gap-3 ml-2" dir="rtl">
