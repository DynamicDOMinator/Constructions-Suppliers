          ...
          return (
            <Link 
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-colors ${
                isActive 
                  ? "text-[#2A5CBA] font-bold border-r-4 border-[#2A5CBA] bg-blue-50/50" 
                  : "text-gray-500 font-medium hover:text-[#2A5CBA] hover:bg-gray-50"
              }`}
            >
              <item.icon className={`w-5 h-5 ${isActive ? "text-[#2A5CBA]" : "text-gray-400"}`} />
              <span>{item.name}</span>
            </Link>
          );
        })}
