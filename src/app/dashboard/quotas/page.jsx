...
import { ChevronDown, MapPin, Building2, Briefcase, FileText, Download } from "lucide-react";

export default function QuotasRequestsPage() {
  const [activeTab, setActiveTab] = useState("accepted"); // 'accepted' or 'rejected'

  // Mock data for requests, with extra fields for BOQ (مقايسة)
  const requests = Array(9).fill(null).map((_, i) => ({
    id: i + 1,
    name: "محمود فتوح",
    projectName: "مشروع تطوير المجمع التجاري",
    projectType: "تجاري",
    location: "السعودية الرياض",
    notes: "نحتاج إلى توريد وتركيب أنظمة التبريد والمكيفات بالكامل للمجمع التجاري الجديد، يرجى الاطلاع على المرفقات والمواصفات الفنية المرفقة.",
    avatar: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80",
    status: activeTab === "accepted" ? "مقبولة" : "مرفوضة",
    files: ["المقايسة الأصلي.pdf", "المواصفات الفنية.docx", "الرسومات الهندسية.dwg"]
  }));
  ...
  return (
    <div className="font-tajawal max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4">
      ...
      <div className="text-right">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">طلبات المقايسة المرسلة</h1>
      </div>
        ...
        {requests.map((req) => (
          <Link href={`/dashboard/pricing/1284795`} key={req.id}>
            <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm hover:shadow-md transition-all h-full flex flex-col">
              ...
              {/* Extra BOQ Details */}
              <div className="flex flex-col gap-3 mb-4 bg-gray-50 p-4 rounded-xl text-right">
                <div className="flex flex-row items-center justify-start gap-2">
                  <Building2 className="w-4 h-4 text-gray-400" />
                  <span className="text-sm font-bold text-gray-700">اسم المشروع:</span>
                  <span className="text-sm text-gray-600">{req.projectName}</span>
                </div>
                <div className="flex flex-row items-center justify-start gap-2">
                  <Briefcase className="w-4 h-4 text-gray-400" />
                  <span className="text-sm font-bold text-gray-700">نوع المشروع:</span>
                  <span className="text-sm text-gray-600">{req.projectType}</span>
                </div>
              </div>

              {/* Attachments Section */}
              <div className="mb-6 flex flex-col  gap-2 text-right">
                <span className="text-xs font-bold text-gray-500 mb-1">المرفقات والوثائق:</span>
                <div className="flex  flex-wrap flex-row gap-2">
                  {req.files.map((file, idx) => (
                    <button key={idx} className="flex items-center gap-1.5 bg-orange-50 border border-orange-100 text-[#EB682C] px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-orange-100 transition-colors">
                      <FileText className="w-3.5 h-3.5" />
                      <span>{file}</span>
                      <Download className="w-3.5 h-3.5 mr-1 text-orange-300 hover:text-[#EB682C]" />
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="text-right flex-1 mb-6">
                <h4 className="font-bold text-gray-900 mb-2">وصف/مواصفات المشروع</h4>
