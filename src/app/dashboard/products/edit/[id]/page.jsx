...
export default function EditProductPage() {
  ...
  const fetchProduct = async () => {
    try {
      ...
      const allProducts = Array.isArray(fetchedData)
        ? fetchedData
        : fetchedData?.data || [];
      const product = allProducts.find((p) => p.uuid === id);
      if (product) {
        ...
        if (product.images && product.images.length > 0) {
          setPreviewUrls(
            product.images
              .filter((img) => img)
              .map((imgUrl) => ({ url: imgUrl, file: null })),
          );
        }
      } else {
        setPopupState({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await api.post(`/auth/company/products/${id}`, formData, {
      ...
      setPopupState({
        ...
        type: "danger",
        title: "خطأ",
        message: err.response?.data?.message || "حدث خطأ أثناء تعديل المنتج",
        onConfirm: () => setPopupState({ ...popupState, isOpen: false }),
      });
    } finally {
      setLoading(false);
