...
export default function EditServicePage() {
  ...
  const fetchService = async () => {
    try {
      const res = await api.get("/auth/company/services");
      const allServices = res.data || [];
      const service = allServices.find((s) => s.uuid === id);
      if (service) {
        setName(service.name || "");
        setDescription(service.description || "");
        if (service.images && service.images.length > 0) {
          setPreviewUrls(service.images.filter(img => img).map((imgUrl) => ({ url: imgUrl, file: null })));
        }
      } else {
        setPopupState({
          isOpen: true,
  ...
  const handleSubmit = async () => {
    if (!name) {
      setPopupState({
        ...
        type: "alert",
        title: "تنبيه",
        message: "الرجاء إدخال اسم الخدمة",
    ...
    try {
      ...
      await api.post(`/auth/company/services/${id}`, formData, {
      ...
      setPopupState({
        ...
        type: "danger",
        title: "خطأ",
        message: err.response?.data?.message || "حدث خطأ أثناء تعديل الخدمة",
        onConfirm: () => setPopupState({ ...popupState, isOpen: false }),
      });
    } finally {
      setLoading(false);
